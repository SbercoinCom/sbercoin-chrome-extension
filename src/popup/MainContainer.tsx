...
const UnexpectedErrorDialog: React.SFC<any> = inject('store')(observer(({ store: { mainContainerStore } }) => (
  ...
  >
    <DialogTitle>{chrome.i18n.getMessage('popup_MainContainer_unexpectedError')}</DialogTitle>
    <DialogContent>
      <DialogContentText>{ mainContainerStore.unexpectedError }</DialogContentText>
    </DialogContent>
    <DialogActions>
      <Button onClick={() => mainContainerStore.unexpectedError = undefined} color="primary">{chrome.i18n.getMessage('popup_pages_login_close')}</Button>
    </DialogActions>
  </Dialog>
)));
