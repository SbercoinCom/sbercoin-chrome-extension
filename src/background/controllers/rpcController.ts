import { WalletRPCProvider, Insight } from 'sberjs-wallet';
  ...
  */
  public sendToContract = async (id: string, args: any[]): Promise<IRPCCallResponse> => {
    ...
    try {
      ...
      if (!rpcProvider) {
        throw Error(chrome.i18n.getMessage('error_cannotSendContractWithoutRPC'));
      }
      if (args.length < 2) {
      ...
      const newArgs = [
        ...
        gasPrice * 1e-7 || DEFAULT_GAS_PRICE,
      ];
  ...
  */
  private externalRawCall = async (id: string, method: string, args: any[]) => {
    ...
    try {
      const rpcProvider = this.rpcProvider();
      if (!rpcProvider) {
        throw Error(chrome.i18n.getMessage('error_cannotCallRPCProvider'));
      }
