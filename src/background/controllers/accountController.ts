...
import { Wallet as SbercoinWallet } from 'sberjs-wallet';
...
export default class AccountController extends IController {
  ...
  */
  public addAccountAndLogin = async (accountName: string, privateKeyHash: string, wallet: SbercoinWallet) => {
  ...
  */
  public saveToFile = (accountName: string, mnemonic: string) => {
    ...
    element.download = `sbercoin_${accountName}_${timestamp}.bak`;
    element.click();
  ...
  */
  public loginAccount = async (accountName: string) => {
    ...
    if (!this.loggedInAccount) {
      throw Error(chrome.i18n.getMessage('error_accountIsUndefined'));
    }
  ...
  */
  private recoverFromPrivateKeyHash(privateKeyHash: string): SbercoinWallet {
  ...
  private getPrivateKeyHash(wallet: SbercoinWallet) {
  ...
  */
  private validatePassword = async (): Promise<boolean> => {
    ...
    if (!isEmpty(this.mainnetAccounts)) {
      ...
      throw Error(chrome.i18n.getMessage('error_tryingToValidate'));
    }
  ...
  */
  private getWalletInfo = async (sendInpageUpdate = true) => {
    if (!this.loggedInAccount || !this.loggedInAccount.wallet || !this.loggedInAccount.wallet.qjsWallet) {
      console.error(chrome.i18n.getMessage('error_couldNotGetWalletInfo'));
      return;
    }
    ...
    if (infoDidUpdate) {
      ...
      this.updateAndSendMaxSbercoinAmountToPopup();
    }
  }
  ...
  * @param receiverAddress The address to send Sbercoin to.
  * @param amount The amount to send.
  */
  private sendTokens = async (receiverAddress: string, amount: number, transactionSpeed: TRANSACTION_SPEED) => {
    if (!this.loggedInAccount || !this.loggedInAccount.wallet || !this.loggedInAccount.wallet.qjsWallet) {
      throw Error(chrome.i18n.getMessage('error_cannotSendWithNoInstance'));
    }
    ...
    */
    try {
      const rates = {
        [TRANSACTION_SPEED.FAST]: 10,
        [TRANSACTION_SPEED.NORMAL]: 5,
        [TRANSACTION_SPEED.SLOW]: 3,
      };
      const feeRate = rates[transactionSpeed]; // greph/byte; 7 greph/byte == .0007 SBER/KB
      if (!feeRate) {
        throw Error(chrome.i18n.getMessage('error_feeRatenotSent'));
      }
  ...
  /**
   * We update the maxSbercoin amount under 2 scnearios
   * 1 - When wallet.info has been updated because a new balance has a new maxSbercoinSend
   * 2 - Whenever the maxSbercoinSend is requested, because even if the balance does not
   * change, the available UTXOs can change(which causes a change in maxSbercoinSend).
   * For instance when a user sends Sbercoin, but that transaction has not confirmed yet,
   * the balance will not change, but calcMaxSbercoinSend is able to account for those
   * unconfirmed UTXOs and update maxSbercoinSend accordingly.
   */
  private updateAndSendMaxSbercoinAmountToPopup = async () => {
    if (!this.loggedInAccount || !this.loggedInAccount.wallet || !this.loggedInAccount.wallet.qjsWallet) {
      throw Error(chrome.i18n.getMessage('error_cannotCalculateMaxBalance'));
    }

    const calcMQSPr = this.loggedInAccount.wallet.calcMaxSbercoinSend(this.main.network.networkName);
    calcMQSPr.then(() => {
      chrome.runtime.sendMessage({ type: MESSAGE_TYPE.GET_MAX_SBER_SEND_RETURN,
        maxSbercoinAmount: this.loggedInAccount!.wallet!.maxSbercoinSend });
    });
  }
    ...
    try {
      switch (request.type) {
        ...
        case MESSAGE_TYPE.GET_WALLET_INFO:
          ...
          break;
        case MESSAGE_TYPE.GET_SBER_USD:
          sendResponse(this.loggedInAccount && this.loggedInAccount.wallet
            ? this.loggedInAccount.wallet.sberUSD : undefined);
          break;
        case MESSAGE_TYPE.VALIDATE_WALLET_NAME:
          sendResponse(this.isWalletNameTaken(request.name));
          break;
        case MESSAGE_TYPE.GET_MAX_SBER_SEND:
          this.updateAndSendMaxSbercoinAmountToPopup();
          break;
        default:
          break;
