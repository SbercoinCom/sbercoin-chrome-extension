...
export default class CryptoController extends IController {
  ...
  public get validPasswordHash(): string {
    if (!this.passwordHash) {
      throw Error(chrome.i18n.getMessage('error_passwordHashShouldBeDefined'));
    }
  ...
  */
  public generateAppSaltIfNecessary = () => {
    try {
      ...
      throw Error(chrome.i18n.getMessage('error_generatingApSalt'));
    }
  }
  ...
  */
  public derivePasswordHash = (password: string) => {
    if (!this.appSalt) {
      throw Error(chrome.i18n.getMessage('error_appSaltShoulNotBeEmpty'));
    }
    ...
    */
    ...
    if (typeof(sww) === 'undefined') {
      ...
      sww.onmessage = (e) => {
        if (e.data.err) {
          throw Error(chrome.i18n.getMessage('error_scryptFailedToCalculatedDerivedKey'));
        }
